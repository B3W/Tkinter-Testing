'''
Example UI demonstrating light/dark theme with both tk and ttk widgets
'''
...
class MainUI(ttk.Frame):
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self.rowconfigure(0, weight=1)  # Lbl, OpMenu, ChkBx, Lbl, Btn
        self.rowconfigure(1, weight=1)  # Lbl, Entry
        self.rowconfigure(2, weight=1)  # LstBx
        ...
        self.__listbox_configure('light')
    ...
    def __create_widgets(self):
        ...
        # Create tk widgets
        self.lst_box = tk.Listbox(self,
                                  relief=tk.FLAT,
                                  selectmode=tk.BROWSE,
                                  activestyle=tk.NONE,
                                  highlightthickness=0)

        self.lst_box.insert(0, 'ITEM')
        self.lst_box.insert(0, 'ITEM')
        self.lst_box.insert(0, 'ITEM')
        ...
        self.lst_box.grid(column=0, row=3, columnspan=2,
                          sticky=(tk.N, tk.S, tk.E, tk.W))

    # CALLBACKS
    def __listbox_configure(self, val):
        if val not in self.styler.style_options:
            return

        if val == 'light':
            self.lst_box.configure(bg=Styler._LIGHT_LIGHT,
                                   fg=Styler._DARK_DARK)

        else:
            self.lst_box.configure(bg=Styler._DARK_GREY,
                                   fg=Styler._LIGHT)

    def __on_option_select(self, val):
        self.styler.configure(val)
        self.__listbox_configure(val)
